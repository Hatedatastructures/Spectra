...
namespace sec::scanner
{
    ...
    public:
        ...
        auto scan_subnet(std::string_view subnet, std::error_code &ec)
            -> net::awaitable<std::vector<device>>;

        auto scan_single(std::string_view target_ip, std::error_code &ec)
    ...
    private:
        ...
        auto scan_subnet_pcap(std::uint32_t start_ip, std::uint32_t end_ip,
                              std::error_code &ec)
            -> net::awaitable<std::vector<device>>;

        auto scan_single_pcap(std::uint32_t target_ip,
                              std::error_code &ec)
            -> net::awaitable<std::optional<device>>;
#endif
