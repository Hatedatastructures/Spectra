...
namespace sec::scanner
{
    ...
    public:
        ...
        auto scan(const std::string &target_ip, const port_scan_options &options, std::error_code &ec)
            -> net::awaitable<std::vector<std::uint16_t>>;

    private:
        auto scan_single_port(const std::string &ip, std::uint16_t port, std::uint16_t timeout_ms)
            -> net::awaitable<bool>;
