...
namespace sec::mitm
{
    ...
    [[nodiscard]] auto arp_detector::check_flood(std::chrono::steady_clock::time_point now,
        const std::string &sender_mac, mac_origin origin) -> std::optional<arp_alert>
    {
        if (origin == mac_origin::remote)
        {
        ...
        -> std::optional<arp_alert>
    {
        ...
        const auto origin = (self_macs_.find(sender_mac) != self_macs_.end())
            ? mac_origin::self : mac_origin::remote;

        prune_bindings(now);

        // ARP flood
        if (auto flood = check_flood(now, sender_mac, origin))
            return flood;
