...
namespace sec::decoder
{
    ...
    namespace
    {
        ...
        bool is_ftp_command(std::string_view token) noexcept
        {
            static constexpr std::string_view commands[] = {
                ...
                "NOOP", "FEAT", "OPTS", "AUTH", "PBSZ", "PROT", "EPSV", "EPRT",
                "MLSD", "MLST"
            };
    ...
    [[nodiscard]] auto ftp_decoder::decode(std::span<const std::byte> payload) noexcept
        -> std::optional<ftp_info>
    {
        ...
        {
            info.type = ftp_message_type::response;
        ...
        else
        {
            ...
            if (!is_ftp_command(cmd_upper))
            {
                return std::nullopt;
            }

            info.type = ftp_message_type::command;
