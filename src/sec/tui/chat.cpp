...
namespace sec::tui
{
    ...
    namespace
    {
        ...
        // 根据 endpoint 和协议计算完整 API 路径（Anthropic 加 /v1/messages，OpenAI 加 /chat/completions）
        auto build_api_path(const std::string &base_path, api_protocol protocol) -> std::string
        {
            auto path = base_path;
            if (protocol == api_protocol::anthropic)
            {
                if (path.find("/v1") == std::string::npos) path += "/v1";
                path += "/messages";
            }
            else
            {
        ...
        // 按协议构造请求体（Anthropic 顶层 system，OpenAI 内嵌 system message）
        auto build_request_body_by_protocol(api_protocol protocol, const std::string &model,
            const std::string &system_prompt, const std::vector<chat_message> &history,
            std::uint16_t max_tokens, float temperature) -> std::string
        {
            if (protocol == api_protocol::anthropic)
            {
                return build_request_body_anthropic(model, system_prompt, history, max_tokens, temperature);
            }
            return build_request_body(model, system_prompt, history, max_tokens, temperature);
        }
    ...
    void ai_chat::send(const std::string &text,
                       std::function<void(std::string_view)> on_chunk,
    ...
    void ai_chat::do_remote_request(const std::string &text,
                                     std::function<void(std::string_view)> on_chunk,
                                     std::function<void()> on_done)
    {
#ifdef _WIN32
        ...
        std::thread{[this, text, on_chunk = std::move(on_chunk), on_done = std::move(on_done)]() mutable
        {
            try
            {
                ...
                auto api_path = build_api_path(parsed.path, remote_cfg_.protocol);
                auto body = build_request_body_by_protocol(
